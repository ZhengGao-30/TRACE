...
// Archived entry records remain supported by copy helpers, but are no longer
// part of the live catalogue. Keep fixtures independent of selectable indices.
...
// Exercise the real picker/banner functions without mounting the 3D app.
...
function appFunction(name) {
  ...
  return Function('constructionPolicyCopy', 'CONSTRUCTION_CASES', `${code}; return ${name}`)(constructionPolicyCopy, CONSTRUCTION_CASES)
}
...
function row(policy_source, sample = CONSTRUCTION_CASES[0]) {
  return { game_id: sample.gameId, task_type: sample.taskType, case_title: sample.title,
    policy_source, run_kind: 'controlled_fault_demo', groups: 31, success: false }
}
...
test('record provenance takes precedence over a stale summary or the controlled task type', () => {
  ...
  assert.match(replayOptionLabel(record, 0), /Controlled fault · Codex LLM replay/)
...
test('both new Codex shift samples remain real recorded options without revealing final outcomes', () => {
  ...
  for (const option of options) {
    ...
    assert.match(label, /31 steps · Controlled fault · Codex LLM replay/)
    assert.doesNotMatch(label, /✓|✗|approved|denied|injury/)
  }
})

test('only the two long shifts remain selectable even with a stale entry-check manifest', () => {
  ...
  assert.ok(options.every((option) => option.groups === 31 && option.task_type === 'construction_ppe_shift' && !option.previewOnly))
...
test('the shipped HSE manifest agrees with the two-sample live catalogue', async () => {
  ...
  assert.ok(hseRows.every((record) => record.groups === 31 && record.task_type === 'construction_ppe_shift'))
})
...
test('the picker preserves authored controlled records and rejects an unverified source', () => {
  const authored = replayOptions([row('scenario_policy')], 'hse')[0]
  assert.equal(authored.previewOnly, false)
  assert.match(replayOptionLabel(authored, 0), /Authored policy replay/)
...
test('controlled banners separate policy provenance from injection and replay limitations', () => {
  for (const source of ['codex_llm', 'api', 'scenario_policy']) {
    ...
    assert.match(note, /Injected controller actions and physical events are excluded.*fault is not a natural LLM failure.*not a live session/)
  }
...
test('landing summaries derive sources from actual duty-shift manifest entries, never case metadata alone', () => {
  ...
  assert.match(codex, /Available duty-shift records: Codex LLM replay.*not token probabilities/)
  ...
  assert.match(mixed, /Available duty-shift sources: Authored policy replay; Codex LLM replay/)
  assert.match(mixed, /not evidence of a natural LLM failure/)
})
